...
export default function SettingsView() {
  ...
  const handleTestSap = () => {
    ...
    setTimeout(() => {
      ...
      setSapTestResult({
        status: 'SANDBOX_OK',
        ping: 'Local Emulator (0ms)',
        message: 'RFC Contract Verified: Skema BAPI_GOODSMVT_CREATE valid. Direct NetWeaver S/4HANA Gateway dijadwalkan pada Phase 2.'
      });
    }, 600);
  };
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      {/* Honest Architecture & Sandbox Status Notice */}
      <div className="p-4 rounded-xl bg-slate-900/80 border border-cyan-500/30 flex items-start gap-3 text-xs">
        <Info className="w-4 h-4 text-cyan-400 shrink-0 mt-0.5" />
        <div className="space-y-1 text-slate-300">
          <div className="font-bold text-white flex items-center gap-2">
            <span>Mode Evaluasi Hackathon (Local Engine + BAPI Contract Emulator)</span>
            <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-cyan-500/10 text-cyan-300 border border-cyan-500/30">Lokal Aktif</span>
          </div>
          <p className="text-slate-400 leading-relaxed">
            Sistem saat ini beroperasi penuh secara lokal menggunakan model AI <strong>Ollama Qwen 2.5</strong> dan bobot <strong>YOLOv8n</strong>. Gateway SAP S/4HANA dan AWS Bedrock di bawah ini terhubung ke <em>Mock BAPI Contract Emulator</em> untuk validasi skema data JSON sebelum provisioning akun cloud resmi dari panitia pada Phase 2.
          </p>
        </div>
      </div>
