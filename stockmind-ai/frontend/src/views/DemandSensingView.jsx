      ...
      <div className="p-4 rounded-2xl bg-gradient-to-r from-cyan-950/40 via-slate-900/60 to-blue-950/40 border border-slate-800 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <option value="4w">Horizon: 4 Minggu (Rekomendasi)</option>
            <option value="8w">Horizon: 8 Minggu</option>
          </select>

          <button
            onClick={handleRunProjection}
            disabled={isProjecting}
            className="px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-400 hover:to-blue-400 text-slate-950 font-bold text-xs flex items-center gap-2 shadow-lg shadow-cyan-500/20 transition-all"
          >
            <RefreshCw className={`w-3.5 h-3.5 ${isProjecting ? 'animate-spin' : ''}`} />
            <span>{isProjecting ? 'Analyzing Series...' : 'Run Demand Projection'}</span>
          </button>
        </div>
      </div>
